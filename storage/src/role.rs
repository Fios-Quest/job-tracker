...
impl Role {
    pub fn new(company_id: Uuid, name: String) -> Role {
        Role {
            id: Uuid::new_v4(),
            company_id,
            name,
            date_applied: SystemTime::now(),
            date_deleted: None,
        }
    }
}
...
mod tests {
    ...
    async fn test_get_by_id<C: Store<Role>>(store: &mut C) {
        let role = Role::new(Uuid::new_v4(), "Test".to_string());
    ...
    async fn test_get_by_name<C: Store<Role>>(store: &mut C) {
        let name = "Test";
        let role = Role::new(Uuid::new_v4(), name.to_string());
    ...
    async fn test_create<C: Store<Role>>(store: &mut C) {
        ...
        let role_same_name = Role::new(Uuid::new_v4(), "Test".to_string());
        assert_eq!(
    ...
    async fn test_delete_by_id<C: Store<Role>>(store: &mut C) {
        ...
        assert!(store.create(role.clone()).await.is_ok());
        assert_eq!(Ok(role.clone()), store.get_by_id(role.id).await);
        assert!(store.delete_by_id(role.id).await.is_ok());
    ...
    async fn test_get_for_company<C: RoleStore>(store: &mut C) {
        ...
        let role1 = Role::new(company1, "Test 1".to_string());
        let role2 = Role::new(company1, "Test 2".to_string());
        let role3 = Role::new(company2, "Test 3".to_string());
        let role4 = Role::new(company2, "Test 4".to_string());
    ...
    mod role {
        ...
        #[test]
        fn test_get_name() {
            let company = Role::new(Uuid::new_v4(), "Test".to_string());
        ...
        fn test_get_and_set_deleted() {
            let mut company = Role::new(Uuid::new_v4(), "Test".to_string());
